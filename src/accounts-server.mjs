...
async function readBody(req) {
  ...
  for await (const chunk of req) { bytes += chunk.length; if (bytes > 65536) fail(413, 'Verzoek is te groot.'); chunks.push(chunk); }
...
export async function createAccountApp({ dataDir = null, legacyStatePath = null, statePath = null, now = () => new Date().toISOString(), legacyFactory, contextKey, securityHeaders } = {}) {
  ...
  const applications = new Map(), preauth = new Map(), authAttempts = new Map();
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (route === '/api/health' && req.method === 'GET') return json(200, { ok: true, mode: 'local_accounts', version: '0.3.0', liveSources: 0, productionReady: false });
      ...
      if (route.startsWith('/api/auth/')) {
        ...
        if (route === '/api/auth/logout' && req.method === 'POST') {
          await auth.logout(token); res.setHeader('Set-Cookie', cookie('omniscout_session', '', 0)); return json(200, { signedOut: true });
        }
  ...
  async function close() {
    return closing ||= (async () => {
      ...
      await publicApp.importQueue.idle(); await manager.close();
    })();
  }
